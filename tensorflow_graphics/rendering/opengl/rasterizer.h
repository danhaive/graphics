...
bool Rasterizer<T>::Create(int width, int height,
  ...
  RETURN_IF_EQ(gl_utils::Program::Create(shaders, &program), false);
  RETURN_IF_EQ(
...
bool Rasterizer<T>::Render(int num_points, absl::Span<T> result) {
  ...
  RETURN_IF_EQ(render_targets_->ReadPixels(result), false);
...
bool Rasterizer<T>::SetShaderStorageBuffer(const std::string& name,
  ...
  if (shader_storage_buffers_.count(name) == 0) {
    ...
    RETURN_IF_EQ(gl_utils::ShaderStorageBuffer::Create(&shader_storage_buffer),
                 false);
  ...
  RETURN_IF_EQ(shader_storage_buffers_.at(name)->Upload(data), false);
...
bool Rasterizer<T>::SetUniformMatrix(const std::string& name, int num_columns,
  ...
  RETURN_IF_EQ(program_->GetResourceProperty(name, GL_UNIFORM, 1, &property, 1,
                                             &uniform_type),
               false);
  ...
  if (type_info == type_mapping.end())  // TODO ALSO check sizes
    return false;
