...
bool dummy_resource_creator(std::unique_ptr<DummyClass> *resource) {
  *resource = std::unique_ptr<DummyClass>(new DummyClass());
  return true;
}

TEST(ThreadSafeResourcePoolTest, Test) {
...
TEST(ThreadSafeResourcePoolTest, TestMultiThread) {
  ...
  std::unique_ptr<DummyClass> local_resources[kNumThreads];
  MemberRoutineWrapper *wrappers[kNumThreads];
